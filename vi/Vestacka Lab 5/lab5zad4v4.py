...
# vraca listu boja uredjenu po heuristici
def lcv_heuristics_order(graph,node,assignments,color_candidates):
...
# vraca nnove dodele i nove domene ako je dodela validna, u suprotnom None
def forward_check(graph,node,assignments,color_candidates,color):
    ...
    return new_assignments,new_candidates
...
def color_graph_backtrack(graph, assignments, node, color_candidates, result, next_node):
    ...
    ordered_colors=lcv_heuristics_order(graph,node,assignments,color_candidates)

    # Backtrack obilazak
    for color in ordered_colors:  # "isprobavamo" redom dodele boja trenutnom cvoru na osnovu LSV
        new_assignments,new_candidates=forward_check(graph,node,assignments,color_candidates,color)
        if new_assignments==None or new_candidates==None:
            continue
