...
def valid_move(state, table, dominoes):
    ...
    for i, j in all_moves["DOWN"]:
        if state[i][j] == 0 and state[i+1][j] == 0:
            valid.append(
                ((i, j, True), dominoes-{(table[i+1][j], table[i][j]), (table[i][j], table[i+1][j])}))
    for i, j in all_moves["RIGHT"]:
        if state[i][j] == 0 and state[i][j+1] == 0:
            valid.append(((i, j, False), dominoes -
...
def new_state(state, move):
    ...
    if dir == True:
        new_state[i][j], new_state[i+1][j] = 1, 1
    if dir == False:
        new_state[i][j], new_state[i][j+1] = 1, 1
